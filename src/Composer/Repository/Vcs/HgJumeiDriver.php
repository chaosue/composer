...
/**
 * @author chaosue<chaosue@landisee.com>
 */
class HgJumeiDriver extends VcsDriver
{
    protected $tags;
    protected $branches;
    ...
    {
        return 'https://packagist.int.jumei.com/jmzip/';
    }
    ...
    public function getDlFileName($identifier)
    {
        return $this->infoCache[$this->rootIdentifier]['name'].'-'.$identifier.'.zip';
    }

    protected function getPhysicDlPath($identifier)
    {
        return dirname($_SERVER['SCRIPT_FILENAME']).DIRECTORY_SEPARATOR.'jmzip'.DIRECTORY_SEPARATOR. $this->infoCache[$this->rootIdentifier]['name'].DIRECTORY_SEPARATOR.$this->getDlFileName($identifier);
    }
    ...
    {
        $dlFile = $this->getPhysicDlPath($identifier);
        $url = $this->getApiUrl() . $this->infoCache[$this->rootIdentifier]['name'].'/'.$this->getDlFileName($identifier);
        if(!file_exists($dlFile))
        {
            $this->createDlFile($identifier);
        }
        return array('type' => 'zip', 'url' => $url, 'reference' => $identifier, 'shasum' => '');
    }

    protected function createDlFile($identifier)
    {
        $filename = $this->getDlFileName($identifier);
        $filedir = strstr($filename,'.', true);
        $destDir = dirname($this->getPhysicDlPath($identifier));
    ...
    {
        if (preg_match('#(^(?:https?|ssh)://(?:[^@]@)?hg\.int\.jumei\.com)#i', $url)) {
            return true;
        }
